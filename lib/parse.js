...
const parsers = {
  ...
  "arithmetic_command": function(cursor) {
    const token = cursor.fetch();
    cursor.move(+1);
    const node = buildNode({
      type: 'ArithmeticCommand',
      tokens: {
        post: fetchAdditionalTokens(cursor, ["whitespace?"])
      },
      value: token.value,
      params: []
    }, token);
  ...
  "command": function(cursor) {
    ...
    while (
      ...
      const arg = getExpression(cursor);
...
function getExpression(cursor) {
  ...
  while(token = cursor.fetch()) {
    if(validTypes.indexOf(token.type) < 0) {
      // Reach end because of unexpected type
      break;
    }
    if(token.type === "open_paren") {
      ...
      tokens.push(getExpression(cursor));
    }
      ...
      let arg = parsers[token.type](cursor);
  ...
  /**
   * Expression of the format `3 > 2`
   */
  if(tokens.length === 3) {
    // We have an expression
    // TODO: Sort tokens into commands here...
    tokens[1].params = [
      tokens[0],
      tokens[2]
    ]
  ...
  /**
   * Expression of the format `3`
   */
  else if (tokens.length === 1) {
    return tokens[0]
  }
  /**
   * Invalid this could be something like `2 * 4 / 2` which is invalid in our parser and must be bracketed
   * `(2 * 4) / 2`
   */
  else {
